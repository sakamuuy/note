...
var writeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if fileName == "" {
			cmd.PrintErrln("File name is required.")
			return
		}
		if err != nil {
			cmd.PrintErrln(err)
		...
		contentsname := db.GetFileContentsByName(fileName)
		runVim(strings.Join([]string{"./contents/", contentsname, ".md"}, ""))
	},
}

func init() {
	writeCmd.Flags().StringP("file", "f", "", "file name.")
...
func runVim(fileName string) (err error) {
